...
class Adam:
    ...
    def __init__(self):
        ...
        self.all_speech = {}  # all idle interactive voice text 所有空闲的交互式语音文本
        self.get_all_speech_init()  # Get all idle interactive voice text 获取所有空闲的交互式语音文本
    ...
    def take_hot_cup(self):
        ...
        def take_cup():
            ...
            self.goto_angles(which, adam_schema.Angles.list_to_obj([-232.8, -14.9, -37.4, 77.5, 106.3, -217.5]), wait=True, speed=angle_speed)
    ...
    def take_coffee_machine(self, composition: dict, formula=None, cmd_sent=False, type=None, need_adjust=False, is_take_espresso_cup=False, is_take_hot_cup=False):
        """
        "latte": {
            "count": 60,
        ...
        if composition:
            ...
            def goto_coffee_pose(coffee_pose):
                ...
                if need_adjust: # Need to remove need_adjust in the future, not using anymore
                    self.goto_point(which, before_coffee_pose, speed=pose_speed, wait=False) #If a hot drink was ordered this motion will move very slow if the last wait in take_hot_cup is False.
            ...
            try:
                ...
                self.goto_point(which, before_coffee_pose, speed=pose_speed, wait=True)
    ...
    def put_cold_cup(self, cup=None, task_uuid=None):
        ...
        self.goto_temp_point(which, z=put_pose.z - 54, wait=True, speed=pose_speed)
    ...
    def take_cold_cup(self):
        ...
        def take_cup():
            ...
            self.goto_gripper_position(which, self.env.gripper_open_pos)  # 先张开夹爪
            self.goto_angles(which, adam_schema.Angles.list_to_obj([233.1, -9.9, -43.6, -73.4, 109.6, 223.0]), wait=True, speed=angle_speed)
            self.goto_temp_point(which, z=cup_pose.z, speed=pose_speed, wait=True)  # 运动到抓杯位置
            self.goto_gripper_position(which, cup_config.clamp, wait=True)  # 闭合夹爪
        ...
        try:
            ...
            self.goto_angles(which, adam_schema.Angles.list_to_obj([209.4, -22.7, -39, -90.3, 89.5, 28.3]), wait=True,
    ...
    def take_foam_cup_judge(self, arm):
        ...
        def take_cup():
            ...
            if which == Arm.left:
                ...
                self.goto_temp_point(which, x=take_pose.x+5, y=take_pose.y+1, z=take_pose.z-100, speed=pose_speed, wait=True)
            else:
                take_pose.roll = -90
                self.goto_point(which, take_pose, speed=pose_speed, wait=False)
                self.goto_temp_point(which, z=take_pose.z - 100, speed=pose_speed, wait=True)
    ...
    def take_foam_cup(self, arm, is_move=True, is_waiting=True):
        ...
        if is_move:
            ...
            if which == Arm.left:
                ...
                self.goto_temp_point(which, x=take_pose+5, y=take_pose.y+1, z=take_pose.z - 100, speed=pose_speed, wait=True)
            else:
                take_pose.roll = -90
                self.goto_point(which, take_pose, speed=pose_speed, wait=True)
                self.goto_temp_point(which, z=take_pose.z - 100, speed=pose_speed, wait=True)
    ...
    def take_espresso_cup(self):
        ...
        def take_cup():
            self.goto_gripper_position(which, self.env.gripper_open_pos)
            self.goto_angles(which, adam_schema.Angles.list_to_obj([-158.8, 30.9, -98.0, 113.5, 45.5, 146.8]), wait=True, speed = angle_speed)
    ...
    def clean_and_put_espresso_cup(self):
        ...
        time.sleep(2)
        self.goto_temp_point(which, x=500, y=10.5, z=300, wait=False, speed=pose_speed)
        self.goto_angles(which, adam_schema.Angles.list_to_obj([-165.2, 86.6, -139.4, 64.0, 36.1, -181.5]), wait=True, speed=angle_speed)
        check_thread = CheckThread(self.env.one_arm(which), self.stop)
        try:
            check_thread.start()
            ...
            check_thread.stop()
        self.goto_angles(which, adam_schema.Angles.list_to_obj([-168.5, 52.4, -99.4, 94.4, 40.4, -213.8]), wait=True, speed=angle_speed)
        self.goto_temp_point(which, z=87, wait=False, speed=pose_speed)
        self.goto_gripper_position(which, 700, wait=True)
        self.goto_point(which, adam_schema.Pose.list_to_obj([494.4, -235.2, 109.2, -161.9, -89.1, 10.8]), wait=False, speed=pose_speed)
        self.goto_angles(which, adam_schema.Angles.list_to_obj([-209.4, -22.6, -39.0, 90.3, 89.5, -28.3]), wait=True, speed=angle_speed)
    ...
    def take_ice(self, delay_time):
        ...
        self.goto_point(which, dispense_pose, wait=True, speed=pose_speed)
    ...
    def clean_foamer(self):
        ...
        angle_speed = 20 #80
        which = Arm.right
        self.goto_point(which, adam_schema.Pose.list_to_obj([557.9, -0.8, 137.9, -62.4, -83.6, -62.9]), wait=True, speed=pose_speed)
        self.goto_point(which, adam_schema.Pose.list_to_obj([643.5, 5.2, 87.4, -30.3, -86.6, -116.6]), wait=True, speed=pose_speed)
        check_thread = CheckThread(self.env.one_arm(which), self.stop)
        try:
            check_thread.start()
            if self.task_status == AdamTaskStatus.making:
        ...
        self.goto_point(which, adam_schema.Pose.list_to_obj([557.9, -0.8, 137.9, -62.4, -83.6, -62.9]), wait=True, speed=pose_speed)
        self.goto_angles(which, adam_schema.Angles.list_to_obj([-162.9, 42.7, -70.3, 0.0, 27.6, 47.1]), wait=True, speed=angle_speed)
    ...
    def put_foam_cup(self, which, wait=False):
        ...
        if which == Arm.left:  
            take_pose.roll = 90
            self.goto_point(which, take_pose, speed=pose_speed, wait=True)
            self.goto_temp_point(which, x=take_pose.x+5, y=take_pose.y+1, z=take_pose.z-100, speed=pose_speed, wait=False)
        else:
            take_pose.roll = -90
            self.goto_point(which, take_pose, speed=pose_speed, wait=True)
            self.goto_temp_point(which, z=take_pose.z - 100, speed=pose_speed, wait=False)
    ...
    def put_hot_cup(self, cup=None, task_uuid=None):
        ...
        self.goto_point(which, temp_pose, speed=pose_speed, wait=False)  # 放杯位置后上方
        self.goto_point(which, put_pose, wait=True, speed=pose_speed)  # 运动到放杯位置上方
        self.goto_temp_point(which, z=put_pose.z - 50, wait=True, speed=pose_speed)
        self.goto_gripper_position(which, self.env.gripper_open_pos, wait=True)  # 张开夹爪
        self.goto_temp_point(which, x=put_pose.x - 200, wait=False, speed=pose_speed)  # 放完向后退
        self.goto_point(which, self.initial_position(which), wait=False, speed=pose_speed)  # 回零点
    ...
    def pour_foam_cup(self, action):
        ...
        try:
            if action == 'right':  # 目前都是ice
                ...
                self.goto_temp_point(Arm.right, z=300, speed=pose_speed, wait=False)

                # Move both arms into a position ready to pour and receive the pour
                def left_action():
                    self.goto_point(Arm.left, adam_schema.Pose.list_to_obj([490.0, 5.0, 350, 90.0, 90.0, 0.0]), wait=True, speed=pose_speed+25)
                
                def right_action():
                    self.goto_point(Arm.right, adam_schema.Pose.list_to_obj([650.0, -15.3, 460.0, 100.1, 90.0, -179.9]), speed=pose_speed, wait=True)  # 右手位置 x = 465, y = -70, z=420
                ...
                self.goto_angles(Arm.right, adam_schema.Angles.list_to_obj([-126.8, 37.5, -60.7, 6.7, 24.3, -91.5]), wait=True, speed=angle_speed)
    ...
    def make_hot_drink(self, formula, sweetness, milk, beans, ice, receipt_number, task_uuid):
        ...
        try:
            ...
            AudioInterface.gtts(
                "receipt {}, your {} is ready.".format('-'.join(list(receipt_number)), formula))
...
# Motions for the New York installation
    ...
    def stainless_cup_pour_foam(self):
        ...
        angle_speed = 50
        which = Arm.right
        self.goto_point(which, adam_schema.Pose.list_to_obj([513.2, -60.1, 183.3, -105.4, 86.4, -49.5]), wait=True, speed=pose_speed)
        time.sleep(0.1)
        self.goto_angles(which, adam_schema.Angles.list_to_obj([-157.9, 35.5, -56.0, 72.0, 17.8, -179.3]), wait=True, speed=angle_speed)
    ...
    def get_all_speech_init(self):
        riddle_list = []
        joke_list = []
        riddle_transition_list = []
        joke_transition_list = []
        dance_transition_list = []
        mood_list = []
        mood_transition_list = []
        coffee_knowledge_list = []
        coffee_knowledge_transition_list = []

        formula_records = CoffeeInterface.get_formula_duration()
        self.all_speech["coffee_introduction"] = formula_records

        idle_records = CoffeeInterface.get_idle_interaction()
        if idle_records:
            for idle_record in idle_records:
                if idle_record['type'] == 'joke':
                    joke_list.append(idle_record)
                elif idle_record['type'] == 'riddle':
                    riddle_list.append(idle_record)
                elif idle_record['type'] == 'joke_transition':
                    joke_transition_list.append(idle_record)
                elif idle_record['type'] == 'riddle_transition':
                    riddle_transition_list.append(idle_record)
                elif idle_record['type'] == 'dance_transition':
                    dance_transition_list.append(idle_record)
                elif idle_record['type'] == 'mood':
                    mood_list.append(idle_record)
                elif idle_record['type'] == 'mood_transition':
                    mood_transition_list.append(idle_record)
                elif idle_record['type'] == 'coffee_knowledge':
                    coffee_knowledge_list.append(idle_record)
                elif idle_record['type'] == 'coffee_knowledge_transition':
                    coffee_knowledge_transition_list.append(idle_record)

        self.all_speech["joke"] = joke_list
        self.all_speech["riddle"] = riddle_list
        self.all_speech["joke_transition"] = joke_transition_list
        self.all_speech["riddle_transition"] = riddle_transition_list
        self.all_speech["dance_transition"] = dance_transition_list
        self.all_speech["mood"] = mood_list
        self.all_speech["mood_transition"] = mood_transition_list
        self.all_speech["coffee_knowledge"] = coffee_knowledge_list
        self.all_speech["coffee_knowledge_transition"] = coffee_knowledge_transition_list

        # logger.info('all_speech:{}'.format(self.all_speech))

    def choose_speech(self, type, formula=None):
        if type == 'coffee_knowledge':
            joke_transition_list = self.all_speech["coffee_knowledge_transition"]
            num_t = random.randint(0, len(joke_transition_list) - 1)
            AudioInterface.gtts(joke_transition_list[num_t]['content'])
            for i in range(5):
                self.check_adam_status('make coffee waiting', status=AdamTaskStatus.making)
                time.sleep(1)
            joke_list = self.all_speech["coffee_knowledge"]
            num = random.randint(0, len(joke_list) - 1)
            AudioInterface.gtts(joke_list[num]['content'])
        elif type == 'mood':
            joke_transition_list = self.all_speech["mood_transition"]
            num_t = random.randint(0, len(joke_transition_list)-1)
            AudioInterface.gtts(joke_transition_list[num_t]['content'])
            for i in range(5):
                self.check_adam_status('make coffee waiting', status=AdamTaskStatus.making)
                time.sleep(1)
            joke_list = self.all_speech["mood"]
            num = random.randint(0, len(joke_list)-1)
            AudioInterface.gtts(joke_list[num]['content'])
        elif type == 'dance_transition':
            dance_list = self.all_speech["dance_transition"]
            num = random.randint(0, len(dance_list)-1)
            AudioInterface.gtts(dance_list[num]['content'])
        elif type == 'coffee_introduction':
            AudioInterface.gtts('Let me tell you the story about this coffee')
            coffee_list = self.all_speech["coffee_introduction"]
            for coffee in coffee_list:
                if coffee['formula'] == formula:
                    AudioInterface.gtts(coffee['introduction'])
        ...
        if conf.get_idle_Interaction()['state']:
            ...
            if random_number < threshold:
                if type == 'hot':
                    ...
                    if num == 0:
                        left_threads = [threading.Thread(target=self.left_round),threading.Thread(target=self.choose_speech, args=(speech_list[speech_num],formula))]
        ...
        try:
            ...
            def left_step1():
                if composition.get(define.Constant.MachineType.foam_machine, {}).get('foam', {}).get('foam_composition', {}).get('foam_coffee', {}):
                    ...
                    foam_composition = deepcopy(composition.get(define.Constant.MachineType.foam_machine, {}).get('foam', {}).get('foam_composition', {}))
            ...
            def right_step1():
                if foam_machine := composition.get(define.Constant.MachineType.foam_machine, {}):
                    ...
                    self.take_coffee_machine(composition.get(define.Constant.MachineType.coffee_machine, {}), formula, sent_flag, is_take_espresso_cup=True)
                    self.check_adam_status('make_cold_drink take_coffee_machine', status=AdamTaskStatus.making)
            ...
            def left_step2():
                if composition.get(define.Constant.MachineType.foam_machine, {}).get('foam', {}).get('foam_composition', {}).get('foam_coffee', {}):
                    time.sleep(8) #The time the left arm waits while the right arm cleans the espresso cup. The smaller the sleep time, the more likely for both arms to collide since the left arm has no idea where the right arm is.
    ...
    def get_composition_by_option(self, formula, cup, sweetness=0, milk=define.MilkType.milk, beans=define.BeansType.high_roast, ice='no_ice') -> dict:
        """
