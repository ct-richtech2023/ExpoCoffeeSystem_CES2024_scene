...
class CoffeeDriver:
    ...
    def query_status(self, pass_null=False):
        ...
        if temp_value == '' and not pass_null:
            # 咖啡机连接失败，通信为空
            # return self.last_status
    ...
    def make_coffee_from_dict(self, make_dict):
        ...
        while check_status := self.query_status(pass_null=True):
            if wait_time > 600:
                raise CoffeeError('Waiting for the coffee machine exceeded 10 minutes. Please check the coffee machine first.')

            if err := check_status.get('error_msg', []):
                # 制作前系统有错误信息，直接抛出异常
                if len(err) == 1 and COFFEE_STATUS.get('error_msg', {}).get('11') in err:
                    call_gtts(err[0])
                    wait_time += 2
                    time.sleep(2)
                    continue
                raise CoffeeError('Sorry, the coffee machine error, {}'.format(','.join(err)))
        ...
        if return_value == b'':
            for i in range(2):
                time.sleep(0.5)
                self.send_control_message('02', content)
            return True
    ...
    def wait_until_completed(self, formula=''):
        """
        等待制作完成
        """
        if formula == 'Cappuccino':
            time.sleep(24)
            return True
        status_set = set()
        start_time = time.time()
        status_set.add(MachineStatus.idle)
        while status_msg := self.query_status(pass_null=True):
            ...
            if status != MachineStatus.making:
                if status == MachineStatus.idle:
                    if MachineStatus.making not in status_set:
                        logger.debug('no making status in {}'.format(status_set))
                        pass  # 防止出现第一次查询状态就失败，此时last_status是idle
                    else:
                        break
                    if time.time() - start_time > 5 * 60:
                        break  # 极端情况，查询状态校验一直不通过，一直为last_status即idle状态
                else:
                    break
            if error_msg:
                # 制作过程中有报错信息，立刻抛出异常
                logger.error(error_msg)
                raise CoffeeError('Failed to make coffee. The coffee machine error is  {}'.format(status_msg))
            time.sleep(1)
        after_status = self.query_status()
        if after_status.get('system_status') != MachineStatus.idle:
            # 制作完成后，咖啡机必须为idle状态
            raise CoffeeError('Sorry, the coffee machine error, {}'.format(after_status))
        err = after_status.get('error_msg', [])
        if len(err) == 1 and COFFEE_STATUS.get('error_msg', {}).get('11') in err:
            call_gtts(err[0])
...
def call_gtts(text):
    params = {'text': text, 'sync': False}
    url = "http://127.0.0.1:9004/audio/gtts"
    try:
...
if __name__ == '__main__':
    # print(FORMULA.keys())
    coffee_driver = CoffeeDriver('/dev/ttyS4')
